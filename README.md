...
- **Flexible Connection**: Connect using full IPs, short notation (e.g., `lhssh 152`), or IPv6 addresses
- **CIDR Notation**: Scan a /24 network with `lhssh -n 10.99.10.0/24`
- **Command Execution**: Run commands on remote hosts directly
- **Colored Output**: Easy-to-read results with optional color coding (respects `NO_COLOR`)
...
- Standard Unix tools: `grep`, `sort`, `xargs`, `awk`, `getent`, `tput`, `realpath`, `mktemp`
...
5. **Connect via IPv6:**
```bash
lhssh fe80::f67d:d2a:686:7400%enp12s0 hostname
```
...
# SSH login username (defaults to current user; fallback: root)
LOGIN_USERNAME='$USER'
...
- IPv6 connection support (full address passthrough)
- CIDR `/24` notation for network scanning (`-n 10.99.10.0/24`)
- `NO_COLOR` environment variable support
- BCS-compliant code style
